...
class NutritionApp(ctk.CTk):
    def __init__(self) -> None:
        ...
        self.db = Database(
            server=r"DESKTOP-JTV1ES3\SQLEXPRESS",  # ← BURAYI KENDİ BİLGİSAYARINIZA GÖRE DEĞİŞTİRİN
    ...
    def show_login(self) -> None:
        ...
        def on_login() -> None:
            ...
            if not username or not password:
                message_label.configure(text="Lütfen kullanıcı adı ve şifreyi gir.")
                return
            try:
                user_id = self.db.authenticate_user(username, password)
                if user_id is None:
                    message_label.configure(text="Kullanıcı adı veya şifre hatalı.")
                else:
    ...
    def show_register(self) -> None:
        ...
        subtitle = ctk.CTkLabel(
            self.register_frame,
            text="Beslenme planını kişiselleştirmek için bilgilerini gir",
            font=ctk.CTkFont(size=14),
            text_color="#bbbbbb",
        )
        ...
        def on_register() -> None:
            ...
            if not username or not password:
                message_label.configure(text="Kullanıcı adı ve şifre zorunludur.")
                return
            ...
            if age <= 0 or height <= 0 or weight <= 0:
                message_label.configure(text="Yaş, boy ve kilo 0'dan büyük olmalıdır.")
                return
            ...
            try:
                ...
                if not success:
                    message_label.configure(text="Bu kullanıcı adı zaten kullanılıyor.")
                    return
    ...
    def show_dashboard(self) -> None:
        ...
        # Ana içerik: sol = ilerleme, sağ = öneri
        ...
        # Son önerilen öğünü sakla
        self._last_suggested_meal = None  # (meal_id, gram, kalori, protein, karbonhidrat)

        # Başlangıçta ilerlemeyi yükle, su takibini güncelle ve öğün geçmişini göster
    ...
    def _build_engine(self) -> Optional[CalorieEngine]:
        ...
        return CalorieEngine(
            ...
            gender=profile.get("Gender", "Erkek"),  # Eski kayıtlar için varsayılan
    ...
    def _update_progress(self, animated: bool = True) -> None:
        ...
        # Arayüzü kilitlememek için basit bir animasyon (arka planda thread)
    ...
    def _suggest_meal(self) -> None:
        ...
        if not meals:
            self.meal_status_label.configure(text="Hedef kategorin için tanımlı öğün bulunamadı.")
            return
        ...
        grams = max(50.0, grams)  # minimum porsiyon sınırı
        ...
        self.meal_status_label.configure(
            text="Bu öğünü bugünkü alımına eklemek için \"Bu Öğünü Kaydet\" butonuna tıkla.",
            text_color="#aaaaaa",
        )
    ...
    def _log_suggested_meal(self) -> None:
        ...
        try:
            ...
            self.meal_status_label.configure(
                text="Öğün bugüne kaydedildi! İlerleme çubukları güncellendi.",
                text_color=self.accent_color,
            )
